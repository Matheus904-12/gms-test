...
describe('US-012-Funcionalidade: Cadastro de Usuário', () => {
  it('Deve fazer o cadastro de campos obrigatórios', () => {
    ...
    // Aguarda a resposta aparecer no DOM
    cy.get('#signup-response', { timeout: 10000 }).should('not.be.empty')
    
    // Primeiro verifica o que está sendo exibido
    cy.get('#signup-response').then(($el) => {
      const responseText = $el.text()
      cy.log('Response text: ' + responseText)
      
      // Verifica se é uma das respostas esperadas
      expect(responseText).to.satisfy((text) => {
        return text.includes('Usuário cadastrado com sucesso') || 
               ...
               text.includes('message') // qualquer resposta da API
      }, 'Resposta da API deve conter uma mensagem válida')
    })
  })
